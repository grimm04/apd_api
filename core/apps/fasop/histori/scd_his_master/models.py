...
class SCD_HIS_MASTER(models.Model):
    id = models.AutoField(primary_key=True) 
    point_number = models.ForeignKey(
        FASOPMASTER, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='point_number', related_name='master_his'
    )
    ...
    class Meta:
        managed = True
        db_table = 'scd_his_master'

    def __str__(self):
        return self.id 

EXPORT_HEADERS = ['id', 'Master','tanggal_awal','msec_awal', 'status_awal', 'tanggal_akhir','msec_akhir','status_akhir','durasi','kesimpulan']
EXPORT_FIELDS = ['id', 'path3text','datum_1','statekey_1','status_1','datum_2','statekey_2','status_2','datum_capture','kesimpulan']
