...
class RefAsetKondisiSerializersList(serializers.ModelSerializer):
    # user_entri = UserDetailSerializerDef(read_only=True, source='id_user_entri')
    class Meta:
        model = RefAsetKondisi
        fields = '__all__'

class CRRefAsetKondisiSerializers(serializers.ModelSerializer):
...
class UDRefAsetKondisiSerializers(serializers.ModelSerializer):
