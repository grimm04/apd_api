...
from .models import RefAsetRuangan



class RefAsetRuanganSerializersList(serializers.ModelSerializer):
    # user_entri = UserDetailSerializerDef(read_only=True, source='id_user_entri')
    class Meta:
        model = RefAsetRuangan
        fields = '__all__'

class CRRefAsetRuanganSerializers(serializers.ModelSerializer):
...
class UDRefAsetRuanganSerializers(serializers.ModelSerializer):
