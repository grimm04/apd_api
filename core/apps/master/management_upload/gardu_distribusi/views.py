...
class GarduDistribusiView(viewsets.GenericViewSet):

    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = RefLokasi.objects.filter(id_ref_jenis_lokasi=10)
    ...
    @extend_schema(
        operation_id='upload_file_gardu_distribusi',
        summary="Management Upload SLD - Upload Temp Excel Gardu Distribusi",
        description="Management Upload SLD - Upload Temp Excel Gardu Distribusi",
        request={
            'multipart/form-data': {
                'type': 'object',
                'properties': {
        ...
        try:
            ...
            for index, row in data.iterrows():
                ...
                if row['ID_PENYULANG']: 
                    ...
                    id_ulp_1  = rf[2]
                data = dict(
                    nama_lokasi=fillnan(value=row['NAMA_GARDU_DISTRIBUSI']),
                    ...
                    id_parent_lokasi=fillnan(value=row['ID_SEGMENT']),
                    tree_jaringan=1,
                    id_ref_jenis_lokasi=10,
                    alamat=fillnan(value=row['ALAMAT']),
                    no_tiang=fillnan(value=row['NO_TIANG']),
    ...
    @extend_schema(
        tags=['master_management_upload'],
        summary="Management Upload SLD - Download Template Excel Gardu Distribusi",
        description="Management Upload SLD - Download Template Excel Gardu Distribusi",
    )
    @action(methods=['get'], detail=False, url_path='download-template', url_name='download-template')
    def download_template(self, request):
        try:
            data = template(name='gardu_distribusi')
            file_path = os.path.join(settings.MEDIA_ROOT, 'template_upload_gardu_distribusi.xlsx')
            ...
            for data in example:
                ...
                sample_upload.write(index, 9, data.get('latitude'))
                sample_upload.write(index, 10, data.get('longitude')) 
                sample_upload.write(index, 11, data.get('no_tiang')) 
            ...
            sample_upload.merge_range('A10:D10', 'KOSONGKAN ID_GARDU_DISTRIBUSI UNTUK EVENT UPLOAD "INSERT"', merge_format)
    ...
    @extend_schema(
        tags=['master_management_upload'],
        summary="Management Upload SLD - Download Data Excel Gardu Distribusi",
        description="Management Upload SLD - Download Data Excel Gardu Distribusi",
    )
    ...
    def download_excel(self, request):
        try:
            ...
            file_path = os.path.join(settings.MEDIA_ROOT, 'LAP MASTER GARDU_DISTRIBUSI.xlsx')
            workbook = xlsxwriter.Workbook(file_path)
            ws_acc = workbook.add_worksheet('DATA GARDU_DISTRIBUSI')
            format = workbook.add_format()
            format.set_bg_color('#D3D3D3')
            format.set_align('left')
            ...
            for message in serializer.data:
                status = message.get('status_listrik')
                if status == '0':
                    status = 'inactive'
                elif status == '1':
                    status = 'active'
                ...
                if message.get('section'):
                    nama_section= message.get('section', {}).get('nama_lokasi') 
                else:
                    nama_section= '' 
                ...
                ws_acc.write(index, 7, nama_section)
                ws_acc.write(index, 8, parent_lokasi)
                ws_acc.write(index, 9, name_parent_lokasi)
                ws_acc.write(index, 10, message['lat'])
                ws_acc.write(index, 11, message['lon']) 
                ws_acc.write(index, 12, message['no_tiang']) 
