...
class ZonaView(viewsets.GenericViewSet):

    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = RefLokasi.objects.filter(id_ref_jenis_lokasi=7)
    ...
    @extend_schema(
        operation_id='upload_file_zona',
        summary="Management Upload SLD - Upload Temp Excel Zone",
        description="Management Upload SLD - Upload Temp Excel Zone",
        request={
            'multipart/form-data': {
                'type': 'object',
                'properties': {
        ...
        try:
            ...
            for index, row in data.iterrows():
                ...
                if row['ID_PENYULANG']:
                    ...
                    id_ulp_1  = rf[2]
                data = dict(
                    nama_lokasi=fillnan(value=row['NAMA_ZONE']),
                    ...
                    id_parent_lokasi=fillnan(value=row['ID_PENYULANG']),
                    tree_jaringan=1,
    ...
    @extend_schema(
        tags=['master_management_upload'],
        summary="Management Upload SLD - Download Template Excel Zone",
        description="Management Upload SLD - Download Template Excel Zone",
    )
    @action(methods=['get'], detail=False, url_path='download-template', url_name='download-template')
    def download_template(self, request):
        try:
            data = template(name='zona')
            file_path = os.path.join(settings.MEDIA_ROOT, 'template_upload_zona.xlsx')
            ...
            for data in example:
                ...
                sample_upload.write(index, 5, data.get('id_parent_lokasi'))
                sample_upload.write(index, 6, data.get('latitude'))
                sample_upload.write(index, 7, data.get('longitude')) 
            ...
            sample_upload.merge_range('A10:D10', 'KOSONGKAN ID_ZONA UNTUK EVENT UPLOAD "INSERT"', merge_format)
            ...
            if  _: 
                ...
                ref_penyulang.merge_range('F2:O2', 'ID_PENYULANG DIGUNAKAN UNTUK MENGISI ID_PENYULANG KETIKA UPLOAD', merge_format)
    ...
    @extend_schema(
        tags=['master_management_upload'],
        summary="Management Upload SLD - Download Data Excel Zone",
        description="Management Upload SLD - Download Data Excel Zone",
    )
    ...
    def download_excel(self, request):
        try:
            ...
            file_path = os.path.join(settings.MEDIA_ROOT, 'SLD MASTER ZONA.xlsx')
            workbook = xlsxwriter.Workbook(file_path)
            ws_acc = workbook.add_worksheet('DATA ZONA')
            format = workbook.add_format()
            format.set_bg_color('#D3D3D3')
            format.set_align('left')
            ...
            for message in serializer.data:
                ...
                if message.get('parent_lokasi'):
                    parent = message.get('parent_lokasi', {})
                    ...
                    print(parent)
                    if parent:
                        nama_trafo_gi= parent.get('parent_lokasi', {}).get('nama_lokasi') 
                    else:
                        nama_trafo_gi= ''  
                else:
                    parent_lokasi = ''
                ...
                ws_acc.write(index, 5, parent_lokasi)
                ws_acc.write(index, 6, name_parent_lokasi)
