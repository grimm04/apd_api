...
class TransTmPenyulangBulan(models.Model):
    id_trans_tm_penyulang_bulan = models.AutoField(primary_key=True)
    ...
    id_ref_lokasi_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_gi', related_name='ref_gi_penyulang_bulan'
    )
    ...
    id_ref_lokasi_trafo_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_trafo_gi', related_name='ref_trafo_gi_penyulang_bulan'
    )
    id_ref_lokasi_penyulang = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_penyulang', related_name='ref_penyulang_bulan'
    ) 
    ...
    class Meta:
        managed = False
        db_table = 'trans_tm_penyulang_bulan'

    def __str__(self):
        return self.id_trans_tm_penyulang_bulan 

EXPORT_HEADERS = ['Tanggal','Gardu Induk','Penyulang', 'Beban Max(MW)','Beban Rata-Rata(MW)','Beban Max Siang(MW)','Beban Max Malam(MW)','Ampere Max(A)','Ampere Rata-Rata(A)','Ampere Max Siang(A)','Ampere Max Malam(A)','Load Faktor']
...
EXPORT_HEADERS_HARIAN = [{
    ...
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Penyulang PerBulan', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
