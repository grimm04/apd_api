...
from .models import TransTmPenyulangHari ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmPenyulangHariFilter
...
class TransTmPenyulangHariViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmPenyulangHari.objects.all()
    serializer_class = serializers.TransTmPenyulangHariSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmPenyulangHariFilter
    ...
    ordering = ['id_trans_tm_penyulang_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Penyulang Hari",
        description="Get Data Opsis - Laporan Beban - Penyulang Hari",
        parameters=[
    ...
    def list(self, request):
        ...
        title = 'Laporan Beban Tegangan Penyulang PerHari'
        queryset = self.filter_queryset(self.get_queryset())   
 
        return get_response(self, request, queryset, 'trans_tm_penyulang_hari.view''trans_tm_penyulang_jam.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)  
  

class LaporanPenyulangHariPDFTESTViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmPenyulangHari.objects.all()
    serializer_class = serializers.LaporanBebanPenyulangHariPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Penyulang Per Hari Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - Penyulang Per Hari Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_penyulang_hari.not_found')  
        ...
        if day_after and day_before: 
            ...
            fname = 'laporan beban penyulang per Hari.pdf' 
