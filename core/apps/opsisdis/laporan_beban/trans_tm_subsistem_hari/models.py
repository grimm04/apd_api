...
class TransTmSubsistemHari(models.Model):
    id_trans_tm_subsistem_hari = models.AutoField(primary_key=True)
    ...
    id_ref_lokasi_subsistem = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_subsistem'
    )   
    ...
    class Meta:
        managed = False
        db_table = 'trans_tm_subsistem_hari'
    

    def __str__(self):
        return self.id_trans_tm_subsistem_hari 

EXPORT_HEADERS = ['Tanggal', 'Sub Sistem', 'Beban Max(MW)','Beban Rata-Rata(MW)','Beban Max Siang(MW)','Beban Max Malam(MW)','Ampere Max(A)','Ampere Rata-Rata(A)','Ampere Max Siang(A)','Ampere Max Malam(A)','Load Faktor']
...
EXPORT_HEADERS_HARIAN = [{
    ...
    'header_row_start': 5,
    'data': [
        {'name': 'Laporan Beban Tegangan Sub Sistem PerHari', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
        {'name': 'Sub Sitem', 'column': 'A2:B2', 'width': 15, 'merge': True,'align':'left','type':'row'},  
