...
from .models import TransTmSubsistemHari ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmSubsistemHariFilter
...
class TransTmSubsistemHariViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistemHari.objects.all()
    serializer_class = serializers.TransTmSubsistemHariSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmSubsistemHariFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['id_trans_tm_subsistem_hari_id__nama_lokasi']  # multi filter field 
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_subsistem_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Subsistem Hari",
        description="Get Data Opsis - Laporan Beban - Subsistem Hari",
        parameters=[
    ...
    def list(self, request):  
        ...
        title = 'Laporan Beban Tegangan Subsistem PerHari'
        queryset = self.filter_queryset(self.get_queryset())   
 
        return get_response(self, request, queryset, 'trans_tm_subsistem_hari.view''trans_tm_penyulang_jam.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)    
  

class LaporanSubSistemHariPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistemHari.objects.all()
    serializer_class = serializers.LaporanBebanSubSistemHarigHariPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - SubSistem Per Hari Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - SubSistem Per Hari Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_subsistem_hari.not_found')  
        ...
        if day_after and day_before:  
            ...
            fname = 'laporan beban SubSistem per Hari.pdf' 
