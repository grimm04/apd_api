...
from .models import TransTmSubsistem    ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD


from . import serializers
from .filters import SearchFilter, LaporanTransTmSubsistemFilter
...
class LaporanTransTmSubsistemViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistem.objects.all()
    serializer_class = serializers.LaporanBebanUp3Serializers  

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = LaporanTransTmSubsistemFilter
    filterset_fields = ['keyword']   # multi filter param 
    search_fields = ['id_trans_tm_subsistem_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_subsistem']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Subsistem Per Jam",
        description="Get Data Opsis - Laporan Beban - Subsistem Per Jam",
        parameters=[
    ...
    def list(self, request):    
        ...
        title = 'Laporan Beban Tegangan Subsistem Perjam'
        queryset = self.filter_queryset(self.get_queryset())    
        
        return get_response(self, request, queryset, 'trans_tm_subsistem_jam.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)     

class LaporanSubSistemPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistem.objects.all()
    serializer_class = serializers.LaporanBebanUp3PDFSerializers  
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - SubSistem Per Jam Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - SubSistem Per Jam Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_subsistem_jam.not_found') 
        ...
        if datum_after and datum_before:  
            ...
            fname = 'laporan beban SubSistem per jam.pdf' 
