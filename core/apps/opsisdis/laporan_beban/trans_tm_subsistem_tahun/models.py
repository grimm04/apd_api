...
class TransTmSubsistemTahun(models.Model):
    id_trans_tm_subsistem_tahun = models.AutoField(primary_key=True)
    ...
    class Meta:
        managed = False
        db_table = 'trans_tm_subsistem_tahun'

    def __str__(self):
        return self.id_trans_tm_subsistem_tahun 


EXPORT_HEADERS = ['Tahun','Sub Sistem', 'Beban Max(MW)','Beban Rata-Rata(MW)','Beban Max Siang(MW)','Beban Max Malam(MW)','Ampere Max(A)','Ampere Rata-Rata(A)','Ampere Max Siang(A)','Ampere Max Malam(A)','Load Faktor']
...
EXPORT_HEADERS_HARIAN = [{
    ...
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Sub Sistem PerTahun', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
