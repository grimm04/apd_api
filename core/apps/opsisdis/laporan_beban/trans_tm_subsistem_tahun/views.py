...
from .models import TransTmSubsistemTahun , EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmSubsistemTahunFilter
...
class TransTmSubsistemTahunViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistemTahun.objects.all()
    serializer_class = serializers.TransTmSubsistemTahunSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmSubsistemTahunFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_subsistem_tahun_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_subsistem_tahun']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Subsistem Tahun",
        description="Get Data Opsis - Laporan Beban - Subsistem Tahun",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan SubSistem PerTahun'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_subsistem_tahun.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)   

class LaporanSubSistemTahunPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmSubsistemTahun.objects.all()
    serializer_class = serializers.LaporanBebaSubSistemTahunPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - SubSistem Per Tahun Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - SubSistem Per Tahun Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_subsistem_tahun.not_found')  
        ...
        if year_after and year_before:    
            ...
            fname = 'laporan beban SubSistem per Tahun.pdf' 
