...
class TransTmTrafoGiBulan(models.Model):
    id_trans_tm_trafo_gi_bulan = models.AutoField(primary_key=True)
    ...
    id_ref_lokasi_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_gi', related_name='ref_gi_bulan'
    )
    ...
    id_ref_lokasi_trafo_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_trafo_gi', related_name='ref_trafo_gi_bulan'
    )
    ...
    class Meta:
        managed = False
        db_table = 'trans_tm_trafo_gi_bulan'
    
    def __str__(self):
        return self.id_trans_tm_trafo_gi_bulan 
...
EXPORT_HEADERS_BULANAN_KTT = [{
    'row_start': 7,
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Trafo GI KTT PerBulan', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
...
EXPORT_HEADERS_BULANAN_NON_KTT = [{
    'row_start': 7,
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Trafo GI KTT PerBulan', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
