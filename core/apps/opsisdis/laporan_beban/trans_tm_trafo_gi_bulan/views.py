...
from .models import TransTmTrafoGiBulan ,EXPORT_HEADERS,EXPORT_HEADERS_BULANAN_KTT,EXPORT_FIELDS,EXPORT_RELATION_FIELD,EXPORT_HEADERS_BULANAN_NON_KTT
from . import serializers
from .filters import SearchFilter, TransTmTrafoGiBulanFilter
...
class TransTmTrafoGiBulanViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmTrafoGiBulan.objects.all()
    serializer_class = serializers.TransTmTrafoGiBulanSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmTrafoGiBulanFilter
    ...
    ordering = ['id_trans_tm_trafo_gi_bulan']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Trafo GI Bulan",
        description="Get Data Opsis - Laporan Beban - Trafo GI Bulan",
        parameters=[
    ...
    def list(self, request):
        ...
        header_caption = EXPORT_HEADERS_BULANAN_KTT if jenis_layanan == 'KTT' else EXPORT_HEADERS_BULANAN_NON_KTT 
        ...
        return get_response(self, request, queryset, 'trans_tm_trafo_gi_bulan.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)  
  

class LaporanTrafoGiBulanPDFTESTViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmTrafoGiBulan.objects.all()
    serializer_class = serializers.LaporanBebanTrafoGiBulanPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - TrafoGi Per Bulan Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - TrafoGi Per Bulan Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_trafo_gi_bulan.not_found')  
        ...
        if month_after and month_before: 
            ...
            # exit()
            fname = 'laporan beban TrafoGi %s per Bulan.pdf' % jenis_layanan
