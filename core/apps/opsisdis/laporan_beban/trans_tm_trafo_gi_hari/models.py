...
class TransTmTrafoGiHari(models.Model):
    id_trans_tm_trafo_gi_hari = models.AutoField(primary_key=True)
    ...
    id_ref_lokasi_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_gi', related_name='ref_gi_hari'
    )
    ...
    id_ref_lokasi_trafo_gi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_trafo_gi', related_name='ref_trafo_gi_hari'
    )
    ...
    i_tgl_max_malam = models.DecimalField(max_digits=18, decimal_places=2, blank=True, null=True)
    ...
    class Meta:
        managed = False
        db_table = 'trans_tm_trafo_gi_hari'

    def __str__(self):
        return self.id_trans_tm_trafo_gi_hari 


EXPORT_HEADERS = ['Tanggal','Gardu Induk','Trafo', 'Beban Max(MW)','Beban Rata-Rata(MW)','Beban Max Siang(MW)','Beban Max Malam(MW)','Ampere Max(A)','Ampere Rata-Rata(A)','Ampere Max Siang(A)','Ampere Max Malam(A)','Load Faktor']
...
EXPORT_HEADERS_HARIAN_KTT = [{
    ...
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Trafo GI KTT PerHari', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
...
EXPORT_HEADERS_HARIAN_NON_KTT = [{
    ...
    'header_row_start': 6,
    'data': [
        {'name': 'Laporan Beban Tegangan Trafo GI NON KTT PerHari', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'},
        {'name': 'Gardu Induk', 'column': 'A2:B2', 'width': 15, 'merge': True,'align':'left','type':'row'},
        {'name': 'Trafo', 'column': 'A3:B3', 'width': 15, 'merge': True,'align':'left','type':'row'},
        # {'name': 'Area', 'column': 'A4:B4', 'width': 10, 'merge': True,'align':'left','type':'row'}, 
        {'name': 'Tanggal', 'column': 'A4:B4', 'width': 10, 'merge': True,'align':'left','type':'row'}, 
