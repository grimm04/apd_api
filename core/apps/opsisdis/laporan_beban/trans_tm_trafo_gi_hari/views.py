...
from .models import TransTmTrafoGiHari ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN_KTT,EXPORT_FIELDS,EXPORT_RELATION_FIELD ,EXPORT_HEADERS_HARIAN_NON_KTT
from . import serializers
from .filters import SearchFilter, TransTmTrafoGiHariFilter
...
class TransTmTrafoGiHariViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmTrafoGiHari.objects.all()
    serializer_class = serializers.TransTmTrafoGiHariSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmTrafoGiHariFilter
    ...
    ordering = ['id_trans_tm_trafo_gi_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Trafo GI Hari",
        description="Get Data Opsis - Laporan Beban - Trafo GI Hari",
        parameters=[
    ...
    def list(self, request):
        ...
        title = 'Laporan Beban Tegangan Trafo PerHari'
        queryset = self.filter_queryset(self.get_queryset())   
        return get_response(self, request, queryset, 'trans_tm_trafo_gi_hari.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)  
  


class LaporanTrafoGiHariPDFTESTViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmTrafoGiHari.objects.all()
    serializer_class = serializers.LaporanBebanTrafoGiHariPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - TrafoGi Per Hari Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - TrafoGi Per Hari Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_trafo_gi_hari.not_found')  
        ...
        if day_after and day_before: 
            ...
            fname = 'laporan beban TrafoGI %s perHari.pdf' % jenis_layanan
