...
from .models import TransTmUidBulan   ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUidBulanFilter
...
class TransTmUidBulanViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidBulan.objects.all()
    serializer_class = serializers.TransTmUidBulanSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUidBulanFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_uid_bulan_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_uid_bulan']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Bulan",
        description="Get Data Opsis - Laporan Beban - UID Bulan",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan UID PerBulan'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_uid_bulan.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)   

class LaporanUIDBulanPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidBulan.objects.all()
    serializer_class = serializers.LaporanBebanUIDBulanPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Per Bulan Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UID Per Bulan Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_uid_bulan.not_found')  
        
        month_after = request.GET['month_after'] if 'month_after' in request.GET else None
        month_before = request.GET['month_before'] if 'month_before' in request.GET else None 
        # print(month_after,month_before) 
        if month_after and month_before:  
            label_child = d[0].get('ref_lokasi_uid').get('nama_lokasi')
            label = 'UID %s' % (label_child) 
            ...
            # exit()
            fname = 'laporan beban UID per Bulan.pdf' 
