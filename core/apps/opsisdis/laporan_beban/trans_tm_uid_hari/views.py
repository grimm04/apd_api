...
from .models import TransTmUidHari  ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD
from . import serializers
from .filters import SearchFilter, TransTmUidHariFilter
...
class TransTmUidHariViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidHari.objects.all()
    serializer_class = serializers.TransTmUidHariSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUidHariFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_uid_hari_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_uid_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Hari",
    ...
    def list(self, request):  
        ...
        return get_response(self, request, queryset, 'trans_tm_uid_hari.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)   
class LaporanUIDHariPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidHari.objects.all()
    serializer_class = serializers.LaporanBebanUIDHarigHariPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Per Hari Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UID Per Hari Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_uid_hari.not_found')  
        ...
        if day_after and day_before:  
            ...
            fname = 'laporan beban UID per Hari.pdf' 
