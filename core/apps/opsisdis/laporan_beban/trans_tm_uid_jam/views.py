...
from .models import TransTmUid    ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD

from . import serializers
from .filters import SearchFilter, LaporanTransTmUidFilter
...
class LaporanTransTmUidViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUid.objects.all()
    serializer_class = serializers.LaporanBebanUp3Serializers  

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = LaporanTransTmUidFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['id_trans_tm_uid_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_uid']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Per Jam",
        description="Get Data Opsis - Laporan Beban - UID Per Jam",
        parameters=[
    ...
    def list(self, request):    
        ...
        title = 'Laporan Beban Tegangan UID Perjam'
        queryset = self.filter_queryset(self.get_queryset())    
        
        return get_response(self, request, queryset, 'trans_tm_uid_jam.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)    

class LaporanUIDPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUid.objects.all()
    serializer_class = serializers.LaporanBebanUIDPDFSerializers  
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Per Jam Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UID Per Jam Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_uid_jam.not_found') 
        ...
        if datum_after and datum_before:   
            ...
            fname = 'laporan beban UID per jam.pdf' 
