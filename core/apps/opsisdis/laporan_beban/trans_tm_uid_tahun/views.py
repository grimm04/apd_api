...
from .models import TransTmUidTahun, EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUidTahunFilter
...
class TransTmUidTahunViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidTahun.objects.all()
    serializer_class = serializers.TransTmUidTahunSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUidTahunFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_uid_tahun_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_uid_tahun']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Tahun",
        description="Get Data Opsis - Laporan Beban - UID Tahun",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan UID PerTahun'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_uid_tahun.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)  


class LaporanUIDTahunPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUidTahun.objects.all()
    serializer_class = serializers.LaporanBebaUIDTahunPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UID Per Tahun Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UID Per Tahun Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_uid_tahun.not_found')  
        ...
        if year_after and year_before:    
            ...
            fname = 'laporan beban UID per Tahun.pdf' 
