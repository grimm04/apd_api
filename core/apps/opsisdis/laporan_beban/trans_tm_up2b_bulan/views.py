...
from .models import TransTmUp2BBulan  ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUp2BBulanFilter
...
class TransTmUp2BBulanViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BBulan.objects.all()
    serializer_class = serializers.TransTmUp2BBulanSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUp2BBulanFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_up2b_bulan_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up2b_bulan']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Bulan",
        description="Get Data Opsis - Laporan Beban - UP2B Bulan",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan UP2B PerBulan'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_up2b_bulan.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)    

class LaporanUP2BBulanPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BBulan.objects.all()
    serializer_class = serializers.LaporanBebanUP2BBulanPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Per Bulan Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UP2B Per Bulan Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_up2b_bulan.not_found')  
        ...
        if month_after and month_before:  
            ...
            # exit()
            fname = 'laporan beban UP2B per Bulan.pdf' 
