...
from .models import TransTmUp2BHari  ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUp2BHariFilter
...
class TransTmUp2BHariViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BHari.objects.all()
    serializer_class = serializers.TransTmUp2BHariSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUp2BHariFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_up2b_hari_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up2b_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Hari",
        description="Get Data Opsis - Laporan Beban - UID Hari",
        parameters=[
    ...
    def list(self, request):  
        ...
        return get_response(self, request, queryset, 'trans_tm_up2b_hari.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)  

class LaporanUP2BHariPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BHari.objects.all()
    serializer_class = serializers.LaporanBebanUP2BHarigHariPDFSerializers 
    ...
    filter_class = TransTmUp2BHariFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_up2b_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up2b_hari']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Per Hari Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UP2B Per Hari Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_up2b_hari.not_found')  
        ...
        if day_after and day_before:  
            label_child = d[0].get('ref_lokasi_up2b').get('nama_lokasi')
            ...
            fname = 'laporan beban UP2B per Hari.pdf' 
