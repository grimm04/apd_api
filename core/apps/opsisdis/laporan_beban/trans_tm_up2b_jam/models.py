...
class TransTmUp2B(models.Model):
    id_trans_tm_up2b = models.AutoField(primary_key=True)
    ...
    id_parent_lokasi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_parent_lokasi' , related_name='up2b_parent_lokasi'
    )
    id_ref_lokasi_up2b = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_ref_lokasi_up2b' , related_name='up2b_ref_lokasi'
    )

    id_lokasi = models.ForeignKey(
        RefLokasi, default=None, on_delete=models.CASCADE, blank=True, null=True, db_column='id_lokasi' , related_name='up2b_lokasi'
    )
    class Meta:
        managed = False
        db_table = 'trans_tm_up2b'
    
    def __str__(self):
        return self.id_trans_tm_up2b 

EXPORT_HEADERS = ['Tanggal','Jam', 'UP2B', 'Arus (A)', 'Daya Aktif (MW)']
...
EXPORT_HEADERS_HARIAN = [{
    ...
    'header_row_start': 5,
    'data': [
        {'name': 'Laporan Beban Tegangan UP2B PerJam', 'column': 'A1:J1', 'width': 15, 'merge': True,'align':'center','type':'row'}, 
