...
from .models import TransTmUp2BTahun, EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUp2BTahunFilter
...
class TransTmUp2BTahunViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BTahun.objects.all()
    serializer_class = serializers.TransTmUp2BTahunSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUp2BTahunFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_trans_tm_up2b_tahun_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up2b_tahun']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Tahun",
        description="Get Data Opsis - Laporan Beban - UP2B Tahun",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan UP2B PerTahun'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_up2b_tahun.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label) 

class LaporanUP2BTahunPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp2BTahun.objects.all()
    serializer_class = serializers.LaporanBebaUP2BTahunPDFSerializers 
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP2B Per Tahun Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UP2B Per Tahun Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if year_after and year_before:    
            ...
            fname = 'laporan beban UP2B per Tahun.pdf' 
