...
from .models import TransTmUp3Jam   ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD

from . import serializers
from .filters import SearchFilter, LaporanTransTmUp3JamFilter
...
class LaporanTransTmUp3JamViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp3Jam.objects.all()
    ...
    filter_class = LaporanTransTmUp3JamFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['id_ref_lokasi_up3_id__nama_lokasi','id_parent_lokasi_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up3']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP3 Per Jam",
        description="Get Data Opsis - Laporan Beban - UP3 Per Jam",
        parameters=[
    ...
    def list(self, request):     
        ...
        title = 'Laporan Beban Tegangan UP3 Perjam'
        queryset = self.filter_queryset(self.get_queryset())    
        
        return get_response(self, request, queryset, 'trans_tm_up3_jam.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption)    
  

class LaporanUP3PDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP3 Per Jam Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - UP3 Per Jam Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_up3_jam.not_found') 
        ...
        if datum_after and datum_before:  
            ...
            fname = 'laporan beban UP3 per jam.pdf' 
