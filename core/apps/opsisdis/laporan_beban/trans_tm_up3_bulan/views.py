...
from .models import TransTmUp3Bulan ,EXPORT_HEADERS,EXPORT_HEADERS_HARIAN,EXPORT_FIELDS,EXPORT_RELATION_FIELD 
from . import serializers
from .filters import SearchFilter, TransTmUp3BulanFilter
...
class TransTmUp3BulanViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]

    queryset = TransTmUp3Bulan.objects.all()
    serializer_class = serializers.TransTmUp3BulanSerializers 

    pagination_class = CustomPagination

    filter_backends = (SearchFilter, DjangoFilterBackend, OrderingFilter)
    filter_class = TransTmUp3BulanFilter
    filterset_fields = ['keyword']   # multi filter param
    search_fields = ['datum','id_ref_lokasi_up3_id__nama_lokasi']  # multi filter field
    ordering_fields = '__all__'
    ordering = ['id_trans_tm_up3_bulan']

    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - UP3 Bulan",
        description="Get Data Opsis - Laporan Beban - UP3 Bulan",
        parameters=[
    ...
    def list(self, request): 
        ...
        title = 'Laporan Beban Tegangan UP3 PerBulan'
        custom_label = None
        ...
        return get_response(self, request, queryset, 'trans_tm_up3_bulan.view', headers=header, relation=relation,
                        fields=fields, title=title,header_caption=header_caption, custom_label=custom_label)     
  

class LaporanUp3BulanPDFViews(viewsets.GenericViewSet):
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTTokenUserAuthentication]
    ...
    @extend_schema(
        methods=["GET"],
        summary="Get Data Opsis - Laporan Beban - Up3 Per Bulan Export Chart PDF",
        description="Get Data Opsis - Laporan Beban - Up3 Per Bulan Export Chart PDF",
        parameters=[
            OpenApiParameter(name='page', description='page number. isi -1 jika mau tanpa pagination.', required=False,
                             type=str, default=1),
            OpenApiParameter(name='limit', description='limit data per page', required=False, type=str, default=10),  
    ...
    def list(self, request):    
        ...
        if not d:
            return not_found('trans_tm_up3_bulan.not_found')  
        ...
        if month_after and month_before: 
            ...
            # exit()
            fname = 'laporan beban Up3 per Bulan.pdf' 
