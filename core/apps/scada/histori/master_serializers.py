...
#histori 
from apps.fasop.histori.scd_his_master.models import SCD_HIS_MASTER    
from apps.fasop.histori.scd_his_master.filters import SCD_HIS_MASTERFilter
...
class FASOPMASTERSerializers(serializers.ModelSerializer):
    ...
    total_his = serializers.SerializerMethodField(source='master_his',read_only=True)
    ...
    def get_histori(self,obj):
        queryset = obj.master_his.all()
        filter = SCD_HIS_MASTERFilter
        filter_q = 'his__'

        return filters_childs(queryset=queryset,filters=filter ,filter_q=filter_q ,serializer=SCDMasterHISTreeSerializers,context={'query_params':self.context.get('query_params')})
     
    def get_total_his(self, obj):
        return obj.master_his.count() 
 

class SCDMasterHISTreeSerializers(serializers.ModelSerializer): 
