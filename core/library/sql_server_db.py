...
class SQLServerDB:
    ...
    def select(self, table, alias=None, field="*"):
        field = "COUNT (*) OVER () AS rowscount, {} ".format(field)
        if isinstance(field, list):
            field = ", ".join(field)
        if alias is None:
            alias = table
        self.q_select = "SELECT {} FROM {} AS {}".format(field, table, alias)
    ...
    def join(self, table, alias, using=None, on=None, join_type=''):
        ...
        if join_type.upper() not in ['LEFT', 'RIGHT', 'INNER']:
            join_type = ''
        q_join = "{} JOIN {} {} {}".format(join_type.upper(), table, alias,
                                           condition)
        if self.q_join:
    ...
    def limit(self, offset=None, limit=None):
        if offset and limit:
            self.q_limit = "LIMIT OFFSET {} ROWS FETCH NEXT {} ROWS ONLY".format(offset, limit)
        elif limit:
            self.q_limit = "FETCH FIRST {} ROWS ONLY".format(limit)
    ...
    def exact_where(self, column, value, operator='AND'):
        ...
        if column and value:
            self.where(
                "{} = '{}'".format(column, escape_string(str(value))),
                operator)
    ...
    def insert(self, table, data, is_ignore=None, is_update_field_id=None):
        ...
        if isinstance(data, dict):
            ...
            for d in data:
                ...
                fields.append(d)
                values.append(
                    'NULL' if data[d] is None else escape_string(
                        str(data[d])))
            ...
            if is_ignore:
                self.sql_insert = "INSERT IGNORE INTO {}({}) VALUES ('{}')".format(
                    table, ", ".join(fields),
                    "', '".join(values))
            elif is_update_field_id:
                on_duplicate = ["{0} = VALUES({0})".format(uf) for uf in
                                update_field]
                self.sql_insert = "INSERT INTO {}({}) VALUES ('{}') ON DUPLICATE KEY UPDATE {}".format(
                    table,
                    ", ".join(
                        fields),
                    "', '".join(
            ...
            for dat in data:
                ...
                for field in fields:
                    try:
                        ...
                        pass
                    value.append(
                        'NULL' if dat[field] is None else escape_string(
                            str(dat[field])))
                    if is_update_field_id:
            ...
            if is_ignore:
                ...
                self.sql_insert = "INSERT INTO {}({}) VALUES {}".format(table,
                                                                        ", ".join(
                                                                            fields),
                                                                        ", ".join(
                                                                            values))
    ...
    def update_set(self, column, value, inc=None):
        ...
        if column and value:  # while value is null
            ...
            if inc is None:
                ...
                self.q_update_set.append(
                    "{0} = {0} - {1}".format(column, value))
            else:
                self.q_update_set.append(
                    "{0} = {0} + {1}".format(column, value))

    def query(self, sql):
        if not re.search('rowscount', sql):
            sql = re.sub('SELECT ', 'SELECT COUNT (*) OVER () AS rowscount, ', sql)
        self.sql = sql
    ...
    def execute(self, commit=True, count=False, utf8mb64=False, debug=False):
        rowscount = None
        ...
        if self.sql_insert:
            ...
            sql = "{} SET {} {}".format(self.sql_update, set_clause,
                                        self.q_where)
        elif self.sql:
            sql = self.sql
        else:
        ...
        rows = self.dict_fetchall(self.cursor)
        rowscount = 0
        if rows:
            rowscount = rows[0]['rowscount']
        ...
        lastrowid = self.cursor.fetchone()
        if lastrowid:
